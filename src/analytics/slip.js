...
const playerNorm=s=>norm(s).replace(/\b(?:[a-z] ){1,}[a-z]\b/g,m=>m.replaceAll(' ',''));
const markets={completions:['player_pass_completions','passing'],'pass completions':['player_pass_completions','passing'],'passing completions':['player_pass_completions','passing'],'rush attempts':['player_rush_attempts','rushing'],'rushing attempts':['player_rush_attempts','rushing'],'rush att':['player_rush_attempts','rushing'],carries:['player_rush_attempts','rushing'],receps:['player_receptions','receiving'],receptions:['player_receptions','receiving'],'rush yds':['player_rush_yds','rushing'],'rushing yards':['player_rush_yds','rushing'],'receiving yards':['player_reception_yds','receiving'],'rec yds':['player_reception_yds','receiving'],'pass yds':['player_pass_yds','passing'],'passing yards':['player_pass_yds','passing'],'passing tds':['player_pass_tds','passing'],points:['player_points','points'],rebounds:['player_rebounds','rebounds'],assists:['player_assists','assists'],hits:['batter_hits','hits'],'home runs':['batter_home_runs','home_runs'],strikeouts:['pitcher_strikeouts','strikeouts'],'shots on goal':['player_shots_on_goal','shots']};
...
export function parseSlip(text,sport){
 ...
 throw Error('Unrecognized pick: '+raw.slice(0,100)+'. Use Player ATTD, Player over 2.5 receps, Player over 48.5 rush yds, Player under 11.5 rush attempts, or Team ML.');
...
export async function matchSlip(slate,text,sport,filter){
 ...
 const results=picks.map(p=>{const wanted=playerNorm(p.player)==='jsn'?'jaxon smith njigba':playerNorm(p.player);const candidates=pools.get(p.category).filter(q=>{const actual=playerNorm(p.market==='h2h'?q.name:q.description||'');return (actual===wanted||actual.split(' ').at(-1)===wanted)&&q.market===p.market&&(!p.outcome||q.name.toLowerCase()===p.outcome)&&(p.point===null||q.point===p.point);});const unique=[...new Map(candidates.map(q=>[q.id,q])).values()];return {raw:p.raw,candidates:unique};});
